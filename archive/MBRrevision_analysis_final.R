...
# ----------------------------------------------------------------------
# --------- Continuous-Time VAR(1) Model (ESM)  ----------------------
# ----------------------------------------------------------------------
...
sc <- .8
pdf("figures_revision/FigureX_CTVAResm.pdf", width = 10*sc, height = 5*sc)
par(mfrow=c(1,2))
...
library(vars)
vars_out <- VARselect(data[,2:5], lag.max = 1)
AIC_var <- vars_out$criteria[1]

tvar.est <- readRDS("TVAR/tvar_est_ESM.RDS")
tvar_resid <- residuals(tvar.est)
...
sigma <- crossprod(tvar_resid) *  (1/N)


tvar.coef <- getParamsTVAR(tvar.est, data = data, digits = 4)

# # https://eml.berkeley.edu/symposia/nsf98/mtar.pdf
n1 <- table(tvar.coef$states)[2]
n2 <- table(tvar.coef$states)[3]
n1*log(det(tvar.coef$r1$psi)) + 2*p*K^2 + 2*K +
  n2*log(det(tvar.coef$r2$psi)) + 2*p*K^2 + 2*K

# But AIC for VAR is scaled by  N

# https://stackoverflow.com/questions/46174383/vars-package-of-r-aic-after-restrict
log(det(sigma)) + (2* p * K^2)/N

kvar <- (4^2)+4
ktvar <- kvar*2 + 1
log(det(sigma)) + 2*ktvar/N

((n1)/(n1 + n2))*log(det(tvar.coef$r1$psi)) + ((n2)/(n1 + n2))*log(det(tvar.coef$r2$psi)) + 2*ktvar/N



# # https://eml.berkeley.edu/symposia/nsf98/mtar.pdf
# log(det(sigma)) + (2* p * K^2 + 2*K)/N

# tsDyn method
t <- tvar.est$t
t * log(det(crossprod(residuals(tvar.est))/t))
# # ----------------------------------------------------------------------
